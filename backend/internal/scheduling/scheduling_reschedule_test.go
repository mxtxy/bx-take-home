...
func TestSchedulingService_RescheduleJob_SuccessSameTechnician(t *testing.T) {
	...
	job, events, err := service.RescheduleJob(context.Background(), testutil.Manager1(), domain.RescheduleJobInput{
		JobID:        jobID,
		TechnicianID: 1,
		StartsAt:     testutil.MustTime(t, "2026-05-12T14:00:00Z"),
	})
	if err != nil {
		t.Fatalf("reschedule: %v", err)
	}
	if job.TechnicianID != 1 || !job.StartsAt.Equal(testutil.MustTime(t, "2026-05-12T14:00:00Z")) || !job.EndsAt.Equal(testutil.MustTime(t, "2026-05-12T16:00:00Z")) {
		t.Fatalf("job = %#v", job)
	}
...
func TestSchedulingService_RescheduleJob_InvalidInput(t *testing.T) {
	...
	tests := []domain.RescheduleJobInput{
		{JobID: 0, TechnicianID: 1, StartsAt: testutil.MustTime(t, "2026-05-12T14:00:00Z")},
		{JobID: 1, TechnicianID: 0, StartsAt: testutil.MustTime(t, "2026-05-12T14:00:00Z")},
		{JobID: 1, TechnicianID: 1, StartsAt: testutil.MustTime(t, "2026-05-12T14:00:01Z")},
	}
...
func TestSchedulingService_RescheduleJob_MissingJob(t *testing.T) {
	service, _ := newTestSchedulingService(t)
	_, _, err := service.RescheduleJob(context.Background(), testutil.Manager1(), domain.RescheduleJobInput{JobID: 999, TechnicianID: 1, StartsAt: testutil.MustTime(t, "2026-05-12T14:00:00Z")})
...
func TestSchedulingService_RescheduleJob_MissingTargetTechnician(t *testing.T) {
	service, database := newTestSchedulingService(t)
	jobID := testutil.InsertScheduledJob(t, database, 1, 1, 1, "2026-05-12T10:00:00Z")
	_, _, err := service.RescheduleJob(context.Background(), testutil.Manager1(), domain.RescheduleJobInput{JobID: jobID, TechnicianID: 999, StartsAt: testutil.MustTime(t, "2026-05-12T14:00:00Z")})
	assertCode(t, err, domain.ErrorNotFound)
}
...
func TestSchedulingService_RescheduleJob_CompletedJobImmutable(t *testing.T) {
	...
	_, _, err := service.RescheduleJob(context.Background(), testutil.Manager1(), domain.RescheduleJobInput{JobID: jobID, TechnicianID: 1, StartsAt: testutil.MustTime(t, "2026-05-12T14:00:00Z")})
...
func TestSchedulingService_RescheduleJob_OverlapConflict(t *testing.T) {
	...
	testutil.InsertScheduledJob(t, database, 2, 1, 1, "2026-05-12T10:00:00Z")

	_, _, err := service.RescheduleJob(context.Background(), testutil.Manager1(), domain.RescheduleJobInput{JobID: job1, TechnicianID: 1, StartsAt: testutil.MustTime(t, "2026-05-12T11:00:00Z")})
	assertCode(t, err, domain.ErrorScheduleConflict)
	if got := scalarString(t, database, `SELECT DATE_FORMAT(starts_at, '%Y-%m-%d %H') FROM jobs WHERE id = ?`, job1); got != "2026-05-12 08" {
...
func TestSchedulingService_RescheduleJob_ExcludesCurrentJobFromOverlapCheck(t *testing.T) {
	service, database := newTestSchedulingService(t)
	jobID := testutil.InsertScheduledJob(t, database, 1, 1, 1, "2026-05-12T10:00:00Z")
	_, _, err := service.RescheduleJob(context.Background(), testutil.Manager1(), domain.RescheduleJobInput{JobID: jobID, TechnicianID: 1, StartsAt: testutil.MustTime(t, "2026-05-12T10:00:00Z")})
	if err != nil {
		t.Fatalf("reschedule: %v", err)
	}
}
...
func TestSchedulingService_RescheduleJob_TargetTechnicianBoundaryAllowed(t *testing.T) {
	...
	job1 := testutil.InsertScheduledJob(t, database, 1, 1, 1, "2026-05-12T08:00:00Z")
	testutil.InsertScheduledJob(t, database, 2, 1, 1, "2026-05-12T10:00:00Z")
	_, _, err := service.RescheduleJob(context.Background(), testutil.Manager1(), domain.RescheduleJobInput{JobID: job1, TechnicianID: 1, StartsAt: testutil.MustTime(t, "2026-05-12T12:00:00Z")})
	if err != nil {
		t.Fatalf("reschedule: %v", err)
	}
}
